...
/* Run a program using given arguments. */
...
char *run_shell_and_get_output(const char *command)
{
    ...
    /* make sure the child process does not become a <defunct> process */
    do {
        process_id = waitpid(child_process_id, &status, 0);
    } while (process_id == -1 && errno == EINTR);

    /* check if the process exited successfully */
    if (process_id == -1 || count < 0 || status != 0) {
        return NULL;
    }
    ...
    (void) memcpy(output, buffer, count);
    output[count] = '\0';
    return output;
}
