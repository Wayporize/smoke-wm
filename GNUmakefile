# C compiler
CC := cc

# Packages
PACKAGES := xcb xcb-errors xcb-xkb xcb-icccm xcb-randr xkbcommon xkbcommon-x11

# Compiler flags
C_FLAGS := -std=c99 -D_XOPEN_SOURCE=700 -Iinclude \
           -Wall -Wextra -Wpedantic -Wno-format-zero-length \
           $(shell pkg-config --cflags $(PACKAGES))
DEBUG_FLAGS := -DDEBUG -DTEST_RANDR -g -fsanitize=address $(C_FLAGS)

# Libraries
C_LIBRARIES := $(shell pkg-config --libs $(PACKAGES))

# Sandbox parameters
SANDBOX_DISPLAY := :8
SANDBOX := Xephyr $(SANDBOX_DISPLAY)

# Find all source files (plus the additional randr test file)
SOURCES := $(shell find src -name '*.c') tests/fake-randr.c
# Get all corresponding object paths
OBJECTS := $(patsubst %.c,build/%.o,$(SOURCES))
# Get all include files
INCLUDES := $(shell find include -name '*.h')

# Get dependencies
DEPENDENCIES := $(patsubst %.o,%.d,$(OBJECTS))

.PHONY: default
default: build

# Include dependencies (.d files) generated by gcc
-include $(DEPENDENCIES)
# Build each object from corresponding source file
build/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) $(DEBUG_FLAGS) -c $< -o $@ -MMD

# Build the main executable from all object files
build/smoke-wm: $(OBJECTS)
	mkdir -p $(dir $@)
	$(CC) $(DEBUG_FLAGS) $(OBJECTS) -o $@ $(C_LIBRARIES)

# Functions
.PHONY: analyze build sandbox clean

analyze:
	gcc -fanalyzer $(DEBUG_FLAGS) $(SOURCES) $(C_LIBRARIES)

build: build/smoke-wm

sandbox: build
	$(SANDBOX) &
	# wait for x server to start
	sleep 1
	DISPLAY=$(SANDBOX_DISPLAY) ./build/smoke-wm
	pkill Xephyr

gdb-sandbox: build
	$(SANDBOX) &
	# wait for x server to start
	sleep 1
	DISPLAY=$(SANDBOX_DISPLAY) gdb -ex run --args ./build/smoke-wm
	pkill Xephyr

clean:
	rm -rf build/
